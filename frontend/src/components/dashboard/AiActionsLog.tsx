import { recentAiActions } from "@/lib/mockData";
  ...
  return (
    <div className="rounded-xl border border-border-light bg-surface-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="text-xs font-medium text-gold-dark hover:underline">
          전체 보기
        </button>
      </div>
        ...
        <tbody>
          {recentAiActions.map((a) => (
            <tr key={a.id} className="border-b border-border-light/60 last:border-0">
              <td className="py-3 font-mono text-xs text-text-muted">{a.time}</td>
              <td className="py-3 text-text-dark">{a.type}</td>
              <td className="py-3 text-text-dark">{a.target}</td>
