...
class GolfTeetime(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    course_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("golf_courses.id"), index=True)
    tee_date: Mapped[date] = mapped_column(Date, nullable=False, index=True)
    ...
    noshow_score: Mapped[float] = mapped_column(Numeric(3, 2), default=0, index=True)
